...
#catch if the user clicks on the windows upper-right 'X' to ensure close
def ask_quit(self):
...
def onSelect(self,event):
    ...
    with conn:
        ...
        #returns a tuple to slice into parts
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.strip() #removes any blank spaces before and after users entry
    ...
    if (len(var_fname) > 0) and(len(var_lname) > 0) and(len(var_phone) > 0) and(len(var_email) > 0 ):
        ...
        with conn:
            cursor = conn.cursor()
            #checks db for existence of fullname, if so, alter user and disregard request
            ...
            if chkName == 0:
                ...
                onClear(self) #calling function to clear all text boxes
            else:
...
def onDelete(self):
    ...
    with conn:
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation", "All information will be permenetly deleted from the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('db_phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
def onDeleted(self):
    #clear all the text in textboxes
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
##onRefresh # update the listbox of changes
    try:
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): #ensures that there is data present
        ...
        with conn:
            ...
            if count == 0 or count2 == 0:
                ...
                print(response)
                if response:
                    #conn = sqlie3.connect('db_phonebook.db')
                    with conn:
                        cursor = conn.cursor()
